...
export default function RentalPhotosPage() {
  ...
  async function loadPhotos() {
    const res = await fetch(`/api/admin/rentals/${id}/photos`);
  ...
  async function handleDelete(photoId: string) {
    await fetch(`/api/admin/rentals/${id}/photos`, {
      method: "DELETE",
      headers: { "Content-Type": "application/json" },
  ...
  return (
    <div className="p-6 max-w-3xl">
      <div className="flex items-center gap-3 mb-6">
        <Link href={`/admin/rentals/${id}/edit`}>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-5 mb-6 space-y-4">
        ...
        <div>
          ...
          <input ref={fileInputRef} type="file" accept="image/*" multiple className="hidden" onChange={handleUploadFile} />
        </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-12 text-center">
          ...
          <p className="text-gray-300 text-xs mt-1">La primera foto será la imagen principal de la galería</p>
        </div>
      ) : (
